...
function footer(){
  return `<div class="footer">Ritter-Kasse – Version 3.0.1 Beta<br>Build 20260629-004<br>© 2026 Jürgen Lindner</div>`;
}
...
function renderExpenseForm(){
  ...
  <section class="card">
    ...
    <label>Betrag</label><input id="eAmount" value="${e?fmtEUR(e.amount):""}" inputmode="decimal" placeholder="z. B. 32,00 oder 32.00"><div class="sub" style="margin-top:6px">Komma oder Punkt sind erlaubt.</div>
    <label>Datum</label><input id="eDate" type="date" value="${e?.date||today()}">
    <label>Bezahlt von</label><select id="ePaidBy">${PEOPLE.map(p=>`<option value="${p.id}" ${p.id===(e?.paidBy||"juergen")?"selected":""}>${p.name}</option>`).join("")}</select>
    ...
    <div class="inline-option" onclick="setCustomSplit(true)"><div class="checkbox-row"><div id="splitCustomBox" class="fakebox ${custom?"checked":""}"></div><div><strong>Abweichende Teilnehmer für diese Ausgabe</strong><br><span class="muted">Für Taxi, getrennte Getränke usw.</span></div></div></div>
    <div id="customParticipants" class="${custom?"":"hidden"}">
      <label>Teilnehmer dieser Ausgabe</label><div class="toggle-list">${PEOPLE.map(p=>toggleHtml("ep_"+p.id,p.name,selectedParts.includes(p.id))).join("")}</div>
    </div>
...
function saveExpense(silent=false){
  ...
  if(custom && !participantIds.length){ toast("Mindestens eine beteiligte Person wählen."); return false; }
  if(editingExpenseId){
...
function renderSettings(){
  return `<section class="card"><h2>Einstellungen</h2>
    <div class="warning">
      Version 3.0.1 Beta speichert alle Daten weiterhin ausschließlich lokal in diesem Browser auf diesem Gerät.
      Es findet noch keine Synchronisation zwischen mehreren Geräten oder Personen statt.
      Die Menüpunkte unten sind teilweise bereits vorgesehen, aber noch nicht alle aktiv. Die Abrechnung wird ab Version 2.1 centgenau mit Ganzzahl-Arithmetik berechnet. Ab Version 2.3 zeigt die App bei Restcent-Aufteilungen einen kompakten Hinweis direkt bei den betroffenen Ausgaben an.
    </div>
  </section>
...
console.log("Ritter-Kasse 3.0.1 Beta (Build 20260629-004)");
